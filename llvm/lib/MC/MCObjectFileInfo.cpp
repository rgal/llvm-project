...
void MCObjectFileInfo::initRepoMCObjectFileInfo(const Triple &T) {
  // Program Repository
  {
    // This is a dummy section simply present to allow the assembler to have a
    ...
    MCSectionRepo *const DummySection = Ctx->getRepoSection(
        "null", MCContext::RepoSection::TextSection, Digest::DigestType{Null});
    DummySection->markAsDummy();
    TextSection = DummySection;
  }
  BSSSection = nullptr; // Ctx->getRepoSection ("null",
                        // MCContext::RepoSection::BSSSection);
