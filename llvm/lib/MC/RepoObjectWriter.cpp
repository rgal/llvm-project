...
public:
  ...
  void writeSectionData(const MCAssembler &Asm, MCSection &Sec,
                        const MCAsmLayout &Layout, TransactionType &Transaction,
                        pstore::index::name_index *const NamesIndex);
...
void RepoObjectWriter::writeRepoSectionData(
    const MCAssembler &Asm, MCSectionRepo &Section, const MCAsmLayout &Layout,
    TransactionType &Transaction, pstore::index::name_index *const NamesIndex) {
...
void RepoObjectWriter::writeSectionData(
    const MCAssembler &Asm, MCSection &Sec, const MCAsmLayout &Layout,
    TransactionType &Transaction, pstore::index::name_index *const NamesIndex) {
...
void RepoObjectWriter::writeObject(MCAssembler &Asm,
  ...
  for (auto &Content : Contents) {
    ...
    if (DigestsIndex->find(Key) != DigestsIndex->end()) {
      ...
      auto Begin = llvm::repo::details::makeSectionContentIterator(
          Content.second.begin());
      auto End =
          llvm::repo::details::makeSectionContentIterator(Content.second.end());

      dbgs() << "fragment " << Key
             << " adding. size=" << repo::Fragment::sizeBytes(Begin, End)
             << '\n';

      pstore::record FragmentRecord =
          repo::Fragment::alloc(Transaction, Begin, End);
