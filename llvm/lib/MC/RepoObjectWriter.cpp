...
void RepoObjectWriter::writeObject(MCAssembler &Asm,
  ...
  for (auto &Fragment : Fragments) {
    ...
    if (DigestsIndex->find(Key) != DigestsIndex->end()) {
      ...
      auto const Kvp = std::make_pair(
          Key, pstore::repo::fragment::alloc(Transaction, Begin, End));
  ...
  for (auto &TicketContent : TicketContents) {
    ...
    // Store the Ticket.
    auto const Kvp =
        std::make_pair(TicketContent.first,
                       pstore::repo::ticket::alloc(Transaction, OutputPathAddr,
                                                   TicketContent.second));
