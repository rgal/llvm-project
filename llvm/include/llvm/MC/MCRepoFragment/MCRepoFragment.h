...
public:
  ...
  static auto alloc(pstore::transaction<LockType> &transaction, Iterator First,
                    Iterator Last) -> pstore::record;

  template <typename Iterator> void populate(Iterator First, Iterator Last) {
  ...
  /// Returns the number of bytes of storage that is required for a fragment
  /// containing the sections defined by [first, last).
  template <typename Iterator>
...
template <typename LockType, typename Iterator>
auto Fragment::alloc(pstore::transaction<LockType> &Transaction, Iterator First,
                     Iterator Last) -> pstore::record {
  static_assert(
