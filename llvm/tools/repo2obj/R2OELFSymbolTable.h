...
public:
  ...
  struct SymbolTarget {
    SymbolTarget(OutputSection<ELFT> const *Section_, std::uint64_t Offset_,
        ...
        : Section{Section_}, Offset{Offset_}, Linkage{Linkage_} {
  ...
  /// the object. \param Linkage  The symbol's linkage. \returns The index of
...
SymbolTable<ELFT>::insertSymbol(pstore::address Name,
  ...
  // have here. Note that SymbolIndex is the index of the symbol in the ELF
  // symbol table. The Symbols_ array does not include the null symbol so we
  // have to subtract 1.
...
SymbolTable<ELFT>::write(llvm::raw_ostream &OS) {
  ...
  for (Value const &SV : Symbols_) {
    ...
    if (SV.Target) {
      ...
      Symbol.st_size =
          0; // FIXME: this code has no idea what the size is (yet).
    } else {
