...
public:
  ...
  std::uint64_t insertSymbol(pstore::address Name, unsigned SectionIndex,
    ...
    // If not already in the symbol table, an undef entry is created. This may
    // be later turned into a proper definition.
    std::uint64_t insertSymbol(pstore::address Name) {
  ...
  private:
    ...
    struct Value {
      ...
      /// The offset of the name of this symbol in the symbol names string
      /// table.
...
SymbolTable<ELFT>::insertSymbol(pstore::address Name,
  ...
  // definition which wouldnt be right.
