...
template <typename T> void writeAlignmentPadding(raw_ostream &OS) {
...
template <typename T> struct StringPolicy {
  std::string get(T const &Str) const;
  std::size_t length(T const &Str) const;
};

template <> struct StringPolicy<std::string> {
  std::string get(std::string const &Str) const { return Str; }
  std::size_t length(std::string const &Str) const { return Str.length(); }
};

template <> class StringPolicy<pstore::address> {
public:
  explicit StringPolicy(pstore::database const &Db) : Db_{Db} {}

  std::string get(pstore::address Addr) const { return getString(Db_, Addr); }
...
template <typename T, typename Policy = StringPolicy<T>> class StringTable {
...
public:
  ...
  explicit StringTable(Policy const &P = Policy()) : Policy_{P} {}
...
StringTable<T, Traits>::write(raw_ostream &OS) const {
...
namespace std {
template <> struct hash<pstore::address> {
...
template <class ELFT> void ELFState<ELFT>::initELFHeader(Elf_Ehdr &Header) {
  ...
  Header.e_shstrndx =
      SectionIndices::SectionNamesStrTab; // getDotShStrTabSecNo();//FIXME:
}
...
template <typename ELFT> void ELFState<ELFT>::initStandardSections() {
  ...
  // last local symbol (binding STRB_LOCAL).
...
public:
  ...
  void append(pstore::repo::ticket_member const &TM,
  ...
  pstore::repo::section_type getType() const { return Type_; }
...
public:
  ...
  struct SymbolTarget {
    SymbolTarget(unsigned SectionIndex_,
                 pstore::repo::section_type SectionType_, std::uint64_t Offset_,
                 pstore::repo::linkage_type Linkage_)
        : SectionIndex{SectionIndex_},
          SectionType{SectionType_}, Offset{Offset_}, Linkage{Linkage_} {}
  ...
  std::uint64_t insertSymbol(pstore::address Name, unsigned SectionIndex,
                             pstore::repo::section_type Type,
                             std::uint64_t Offset,
                             pstore::repo::linkage_type Linkage) {
    return this->insertSymbol(
        Name, SymbolTarget{SectionIndex, Type, Offset, Linkage});
  }

  // If not already in the symbol table, an undef entry is created. This may be
  // later turned into a proper definition.
  std::uint64_t insertSymbol(pstore::address Name) {
...
private:
  ...
  // FIXME: this pair of fields is replicated for the section table.
  // Refactoring?
...
SymbolTable<ELFT>::insertSymbol(pstore::address Name,
  ...
  // have here.
  // FIXME: if Target && V.Target, we're attemptting to make a duplicate
  // definition.
...
template <typename ELFT>
std::tuple<std::uint64_t, std::uint64_t>
SymbolTable<ELFT>::write(raw_ostream &OS) {
  ...
  for (Value const &SV : Symbols_) {
    ...
    if (SV.Target) {
      ...
      auto const SymbolType =
          (T.SectionType == pstore::repo::section_type::Text) ? ELF::STT_FUNC
                                                              : ELF::STT_OBJECT;
      Symbol.setBindingAndType(Binding, SymbolType);
      Symbol.st_shndx = T.SectionIndex; // The section (header table index) in
                                        // which this value is defined.
    } else {
...
SectionMap const SectionAttributes{
    ...
    std::make_pair(pstore::repo::section_type::Data,
                   std::make_tuple(".data", ELF::SHT_PROGBITS,
                                   ELF::SHF_ALLOC | ELF::SHF_WRITE)),
    // X (RelRo)
    std::make_pair(pstore::repo::section_type::Text,
                   std::make_tuple(".text", ELF::SHT_PROGBITS,
...
void OutputSection<ELFT>::append(pstore::repo::ticket_member const &TM,
  ...
  // FIXME: this assumes that the section represents the fragment's one and only
  // section. When we begin to generate multi-section fragments, this will need
  // to be a little more sophisticated. We'll also need to be able to add
  // non-database-resident names to the symbol table.
  dbgs() << "generating relocations FROM " << getString(*Db_, TM.name) << '\n';
  Symbols.insertSymbol(TM.name,
                       typename SymbolTable<ELFT>::SymbolTarget{
  ...
  for (pstore::repo::external_fixup const &Fixup : XFixups) {
    ...
    dbgs() << "generating relocation TO '" << getString(*Db_, Fixup.name)
...
void OutputSection<ELFT>::write(
  ...
  if (Relocations_.size() > 0) {
    ...
    OS.write(reinterpret_cast<char const *>(Relocations_.data()), RelocsSize);
    {
      ...
      RelaSH.sh_flags =
          ELF::SHF_INFO_LINK; // sh_info holds index of the target section.
...

  {
    ...
    for (auto const &TM : *Ticket) {
      ...
      if (FragmentPos == FragmentIndex->end()) {
        errs() << "Error: fragment {" << TM.digest.high() << ','
               << TM.digest.low() << "} was not found.\n";
        return EXIT_FAILURE;
      }
