...
public:
  explicit OutputSection(pstore::database &Db, pstore::repo::section_type T,
                         unsigned Index)
      : Db_{Db}, Type_{T}, Index_{Index} {}
  ...
  std::uint64_t relocationsSize() const {
    ...
    /// \tparam OutputIt  An output iterator to which will be written one or
    /// more instance of the Elf_Shdr type. \param OS The binary output stream
    /// to which the contents of the output sections are written. \param
    /// SectionNames \param NumSections \param OutShdr  An output iterator to
    /// which will be written one or more instance of the Elf_Shdr type as the
    /// write function creates sections in the output.
  ...
  private:
    ...
    // TODO: We have no a priori knowledge of the number of text contributions
    // to this output section. Using some sort of "chunked vector" might be
...
OutputIt OutputSection<ELFT>::write(llvm::raw_ostream &OS,
  ...
  if (Relocations_.size() > 0) {
    ...
    OS.write(reinterpret_cast<char const *>(Relocations_.data()), RelocsSize);
    {
      ...
      RelaSH.sh_flags = llvm::ELF::SHF_INFO_LINK; // sh_info holds index of the
                                                  // target section.
